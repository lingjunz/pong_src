...
from replace.trpo_mpi_utils import traj_segment_generator
...
class PPO1_model_value(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, timesteps_per_actorbatch=256, clip_param=0.2, entcoeff=0.01,
                 ...
                 save_victim_traj=False, save_trajectory = False):
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            with self.sess.as_default():
                ...
                while True:
                    ...
                    logger.log("********** Iteration %i %i************" % (iters_so_far, self.n_envs))
                    ...
                    if self.save_victim_traj and len(obs_opp_list) <= 50:
                        
                        obs_opp_list.append(obs_opp_ph)
                        act_opp_list.append(action_oppo_ph)

                    if self.save_trajectory:
                        obs_list.append(obs_ph)
                        act_list.append(action_ph)
                        rwds_list.append(rwds_ph)
                        dones_list.append(dones_ph)


                    # todo modify obs_ph to obs_mask_ph
                    obs_mask_ph = self.infer_obs_mask_ph(obs_ph)
                    action_mask_ph, _, _, _, _ = self.policy_pi.step(obs_mask_ph)
                ...
                if self.save_victim_traj:
                    length = np.vstack(obs_opp_list).shape[0]
                    print("Save oppo. trajectory for {}...(length:{})".format(self.name, length))
                    oppo_trajectory_dic = {
                        "oppo_obs":np.vstack(obs_opp_list).reshape(length,-1),
                        "oppo_act":np.vstack(act_opp_list).reshape(length,-1)
                    }
                    print(oppo_trajectory_dic['oppo_obs'].shape)
                    print(oppo_trajectory_dic['oppo_act'].shape)

                    joblib.dump(oppo_trajectory_dic,"{}victim_traj.data".format(self.model_saved_loc))


                if self.save_trajectory:
                    
                    length = np.vstack(obs_list).shape[0]
                    print("Save trajectory for {}...(length:{})".format(self.name, length))
                    trajectory_dic = {
                    ...
                    print(trajectory_dic['all_obvs'].shape)
                    print(trajectory_dic['all_acts'].shape)
                    print(trajectory_dic['all_rwds'].shape)
                    print(trajectory_dic['all_dones'].shape)
