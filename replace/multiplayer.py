...
class SharedMemoryClientEnv:
    ...
    def shmem_client_send_env_id(self):
        ...
        '''
        self.pre_fix = self.prefix[:-2]
        self.sh_rew0 = np.memmap(self.pre_fix + "00_rew", mode="r", shape=(1,), dtype=np.float32)
        self.sh_rew1 = np.memmap(self.pre_fix + "01_rew", mode="r", shape=(1,), dtype=np.float32)
        '''
    ...
    def shmem_client_step(self, a):
        ...
        # add here
        #self.reward = np.memmap("/tmp/multiplayer_pongdemo_adv1_player00_rew", mode="r+", shape=(1,), dtype=np.float32)
    ...
    def shmem_client_reset(self):
        ...
        """
        print('Enter this fold')
        os.write(self.sh_pipe_actready, b'R\n')
        check = self.sh_pipe_obsready.readline()[:-1]
        print('Leave this fold')
        if check=='o':
            return self.sh_obs
        else:
...
class SharedMemoryPlayerAgent:
    ...
    def step_and_push_result_tuple(self):
        ...
        if not self.need_response_tuple:
            return
        state, reward, done, info = self.env.step(self.sh_act)
        if done:
            self.passive = True
            self.done = True
            self.need_reset = True
