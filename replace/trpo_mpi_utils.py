# /home/lingjunz/anaconda3/envs/pong/lib/python3.6/site-packages/stable_baselines/trpo_mpi/utils.py
...
def traj_segment_generator(policy, env, horizon, reward_giver=None, gail=False):
    ...
    hiddens = [None for _ in range(horizon)]
    # hiddens = []
    ...
    while True:
        prevac = action
        action, vpred, states, _, last_pi = policy.step(observation.reshape(-1, *observation.shape), states, done)
        ...
        if done:
            ...
            if not isinstance(env, VecEnv):
                observation = env.reset()
        step += 1
        # print("Timestep:{} \n Player: 0 \n Action: {} \n Obs: {} \n Reward:{} \n Done:{}\n".format(step,clipped_action,observation,rew,done))
