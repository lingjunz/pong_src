# /home/lingjunz/anaconda3/envs/pong/lib/python3.6/site-packages/stable_baselines/common/policies.py
...
class ActorCriticPolicy(BasePolicy):
    ...
    @property
    def pi_latent(self):
        """tf.Tensor: the last hidden features, of shape (self.n_batch, last_layer_dim)"""
        return self._pi_latent
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            action, value, neglogp, last_pi = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp, self.pi_latent],
                                                   {self.obs_ph: obs})
        else:
            action, value, neglogp, last_pi = self.sess.run([self.action, self.value_flat, self.neglogp, self.pi_latent],
                                                   {self.obs_ph: obs})
        return action, value, self.initial_state, neglogp, last_pi
