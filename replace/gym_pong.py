...
class PongScene(Scene):
    ...
    def episode_restart(self):
        ...
        if self.score_right + self.score_left > 0:
            ...
            sys.stdout.flush()
        print("\n%i:%i " % (self.score_left, self.score_right))
        print(self.score_board)
        self.mjcf = self.cpp_world.load_mjcf(
            os.path.join(os.path.dirname(__file__), "models_robot/roboschool_pong.xml"))
        dump = 0
        for r in self.mjcf:
        ...
        self.score_left = 0
        self.score_right = 0
        # Add by xian, clear the score_board
        self.score_board = {"left": {"oppo_miss_start": 0,
                                     "oppo_slow_ball": 0,
                                     "oppo_miss_catch": 0,
                                     "oppo_double_hit": 0,
    ...
    def restart_from_center(self, leftwards):
        ...
        self.episode_time = self.MAX_EPISODE_TIME
    ...
    def HUD(self, a, s):
        ...
        s = "%06i LEFT%3i %0.2f:%0.2f BC%2i" % (
            self.frame, self.timeout, self.score_left, self.score_right, self.bounce_n
        )
        score_detail = "L: [{0},{1},{2},{3},{4}], " \
                       "R: [{5},{6},{7},{8}]".format(self.score_board['left']['oppo_miss_start'],
                                                     self.score_board['left']['oppo_slow_ball'],
                                                     self.score_board['left']['oppo_miss_catch'],
                                                     self.score_board['left']['oppo_double_hit'],
                                                     self.score_board['left']['not_finish'],
                                                     self.score_board['right']['oppo_miss_start'],
                                                     self.score_board['right']['oppo_slow_ball'],
                                                     self.score_board['right']['oppo_miss_catch'],
                                                     self.score_board['right']['oppo_double_hit'])
        self.cpp_world.test_window_score(s + " " + score_detail)
        self.camera.test_window_score(s + " " + score_detail)
...
class RoboschoolPong(gym.Env, SharedMemoryClientEnv):
    ...
    def step(self, a):
        ...
        return state, sum(self.rewards), False, {'score_board': self.scene.score_board}
